    ...
    public class Program
    {
            ...
            while (cmd != "QUIT" && cmd != "EXIT" && cmd != "END" && cmd != "X")
            {
                ...
                switch (cmd)
                {
                    ...
                    case "BLIST":
                        ...
                        Utilities.ListProg(engine, arglist[1..], true); break;
                    case "LIST":
                        if (!Utilities.checkLoaded("LIST", engine)) return;
                        Utilities.ListProg(engine, arglist[1..], false); break;
                    case "L.IF":
                    case "LISTIF":
                ...
                if (symInfo.Name.EndsWith("()"))
                {
                    if (engine.DimLines.TryGetValue(symInfo.Name, out var lines))
                    {
                        ...
                        foreach (DimInfo dimInfo in lines)
                        {
                            ...
                            Utilities.ListProg(engine, new string[] { dimInfo.LineNumber.ToString() }, false);
                        }
                    }
                }
                Console.ForegroundColor = ConsoleColor.White;
        ...
        static void ProcFnDetail(BasToolsEngine engine, string[] arglist, bool analysed, string prefix, string cmd)
        {
            ...
            if (defLine != null)
            {
                ...
                Utilities.ListProg(engine, new string[] { lineNumber.ToString() }, false);
            }
            else
            {

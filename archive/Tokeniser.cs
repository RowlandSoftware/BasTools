...
namespace BasTools.Core
{
    public class Tokeniser
    {
        ...
        private static MatchKind DoCompare(ReadOnlySpan<char> slice, ReadOnlySpan<char> kw, out int advanceby)
        {
            if (slice.SequenceEqual(kw))
            {
                advanceby = kw.Length;
                return MatchKind.FullKeyword;
            }
            ...
            if (x > 0)
            {
                ...
                advanceby = x+1;
                if (slice.SequenceEqual(kp))
                    return MatchKind.Abbreviation;
            }
            else
            {
                advanceby = 0;
